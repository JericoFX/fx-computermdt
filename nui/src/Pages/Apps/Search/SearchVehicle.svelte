<script lang="ts">
	...
	async function viewActiveBolo() {
		...
		try {
			await fetchNui('getDataByPlate', {plate: Data.plate}).then((cb) => {
				if (cb) {
	...
	<div bind:this={content} class="window absolute-center" style="max-width:80vw;max-height:80vh;">
		...
		<div class="window-body" style="width:95%;height:95%">
			<div class="field-row row wrap" style="justify-content:space-around;">
				<fieldset>
					...
					<fieldset class="float-left">
						...
						<div class="field-row">
							<span class="label">{$_('page.home.app.vehicle')}:</span>
							<input bind:value={Data.vehicle} disabled type="text" class="input text-center" name="" id="" />
						</div>
						<div class="field-row">
							<span class="label">{$_('category')}:</span>
							<input bind:value={Data.category} disabled type="text" class="input text-center" name="" id="" />
						</div>
						<div class="field-row">
							<span class="label">{$_('color')}:</span>
							<input disabled type="text" class="input text-center" name="" id="" style={`background:${colors[Data.color ?? 1].Hex}`} />
						</div>
						<div class="field-row">
							<span class="label">{$_('owner')}:</span>
							<input bind:value={Data.name} disabled type="text" class="input text-center" name="" id="" />
						</div>
						<div class="field-row">
							<span class="label">{$_('last-name')}:</span>
							<input bind:value={Data.lastname} disabled type="text" class="input text-center" name="" id="" />
						</div>
						<div class="field-row">
							<span class="label">CitizenID:</span>
							<input disabled bind:value={Data.citizenid} type="text" class="input text-center" name="" id="" />
						</div>
					</fieldset>
				</fieldset>
			</div>
			<section class="field-row" style="display: flex;align-items: center;flex-direction: row;flex-wrap: nowrap;justify-content: space-around;">
				{#if Data.hasbolo === 1}
					<button on:click={viewActiveBolo}>{$_('view-bolo')}</button>
				{:else}
					<button on:click={addBolos}>{$_('add-bolo')}</button>
				{/if}
