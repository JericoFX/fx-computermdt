...
<div
	class="jerico absolute-center flex column wrap hide-scrollbar scroll full-height"
>
	<Accordion>
		{#each $Notify as d}
			...
			<div
				transition:slide
				class="accordion-container relative-position q-mt-sm "
			>
				<Panel
					color={d.taked === true ? "seconday" : "primary"}
					variant="outlined"
					square
				>
				...
				>
					...
					{#if d.callsign === $UserInfo.callsign}
						<div class="field-row">
							<img
								class="btn  q-px-sm"
								src="iconos/delete.svg"
								on:click={() => deleteRow(d)}
								alt=""
							/>
						</div>
					{/if}
